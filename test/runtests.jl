...
@testset "FF5Regression.jl" begin
    ...
    df1 = FF5Regression.generate_ff_monthly_data()
    @test ncol(df1) == 7
    ...
    # Testing get_monthly
    df2 = FF5Regression.get_monthly("amd","1963-07-01", now())
    @test ncol(df2) == 2
    @test df2[1,1] == Date(1985,2,1)
    @test round(df2[6,2],digits=6) == 0.144231

    # Testing get_main_data
    df3 = FF5Regression.get_main_data("amd","1963-07-01", now())
    @test ncol(df3) == 9
    ...
    df4 = FF5Regression.model_results_df(Base.vect("aapl", "amd"), "1963-07-01", now(), "FF3")
    @test ncol(df4) == 6
    @test FF5Regression.model_results_df(Base.vect("aapl", "amd"), "1963-07-01", now(), "FF4") == "Incorrect model entered"

    # Testing model_results_dict
    dict1 = FF5Regression.model_results_dict(Base.vect("aapl", "amd"), "1963-07-01", now())
    @test length(dict1) == 2
